...
## URI Validation
The main use that we expect you'll have for the `URI` class in ayacore is validating and parsing bitcoin URIs. A `URI` instance exposes the address as a ayacore `Address` object and the amount in Satoshis, if present.
